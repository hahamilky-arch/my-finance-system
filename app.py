...
def get_data(target_date, all_dates, market_type):
    ...
    df_current['rs_score'] = pd.to_numeric(df_current['rs_score']) # No4용
    ...
    df_vol = pd.DataFrame(supabase.table("stock_prices").select("ticker, volume, price_date").in("ticker", ticker_list).order("price_date", desc=True).limit(400).execute().data)
...
st.markdown('<p style="font-size:24px; font-weight:bold;">📈 모멘텀 분석 (전략 No3 & No4)</p>', unsafe_allow_html=True)
...
if df_display is not None:
    ...
    with tab4:
        st.subheader("전략 No3: 주도주 에너지 폭발")
    ...
    with tab5:
        st.subheader("🚀 No4: High-Octane 전략")
        # No4 필터: 순위 100위 내 + RS 0.4 이상 + 거래량 2.0배
