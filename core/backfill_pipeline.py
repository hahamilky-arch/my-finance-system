...
def backfill_pipeline(start_date_str, end_date_str, market='KR'):
    ...
    # 1. 종목별 루프를 돌며 주가 및 기술적 지표 데이터 수집 (건수 제한 회피)
    ...
    for ticker in target_tickers:
        try:
            # OHLCV 및 ATR, MA50, MA200 등 확장된 컬럼 조회
    ...
    for current_date in date_range:
        ...
        # 해당 날짜의 원본 데이터를 쉽게 조회하기 위한 맵 생성
        ...
        for ticker in target_tickers:
            ...
            # 당일 추가 지표 추출 (해당일 데이터가 존재하는 경우)
            row_data = sub_today.loc[ticker] if ticker in sub_today.index else {}
            
            # MA10, MA20 등 대시보드에서 활용하는 단기 이동평균이 필요할 경우 피벗 데이터로 직접 계산 가능
            # 여기서는 pivot_df를 이용해 10일, 20일 이동평균 산출
            ma10_val = pivot_df[ticker].rolling(window=10).mean().iloc[-1] if len(pivot_df) >= 10 else 0.0
            ma20_val = pivot_df[ticker].rolling(window=20).mean().iloc[-1] if len(pivot_df) >= 20 else 0.0
            
            analysis_data.append({
                "ticker": ticker,
                "rs_score": float(rs_val) if pd.notna(rs_val) else 0.0,
                "rs_score_10": float(row_data.get('rs_score_10', 0.0)) if pd.notna(row_data.get('rs_score_10', 0.0)) else 0.0,
                ...
                "ma10": float(ma10_val) if pd.notna(ma10_val) else 0.0,
                "ma20": float(ma20_val) if pd.notna(ma20_val) else 0.0,
                "price_date": d_str,
                "market": market
            })
